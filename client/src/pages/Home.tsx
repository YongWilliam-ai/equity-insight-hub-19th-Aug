      ...
      <main>
            ...
            <div className="mt-8 grid gap-7 xl:grid-cols-[1.12fr_0.88fr]">
                ...
                <div className="mt-5 divide-y divide-[#E1D7C5] border-t border-[#E1D7C5]">
                  {visibleSignals.map((signal: MarketSignal, index: number) => (
                    <motion.button key={signal.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.035 }} onClick={() => setSelectedSignal(signal)} className={`group grid w-full grid-cols-[34px_1fr_auto] gap-3 py-5 text-left transition ${selectedSignal?.id === signal.id ? "bg-[#EBE5D9] px-3" : "hover:bg-[#F0EBE0] hover:px-3"}`}>
                      ...
                      <span><span className="flex flex-wrap items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#658078]">{signal.category}</span><span className="text-[11px] text-[#887C69]">{signal.move}</span></span><span className="mt-1 block font-display text-[22px] leading-tight text-[#16382F]">{signal.signal}</span><span className="mt-2 block max-w-xl text-sm leading-6 text-[#6A756F]">{signal.thesis}</span></span>
              ...
              <div className="relative min-h-[490px] overflow-hidden bg-[#153A32] p-7 text-[#F7F0E3] sm:p-9">
                ...
                <div className="relative flex h-full flex-col">
                  <div className="flex items-center justify-between border-b border-white/15 pb-4"><span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#E0C27B]">Research drawer</span><Sparkles size={17} className="text-[#E0C27B]" /></div>
                  {selectedSignal && <AnimatePresence mode="wait"><motion.div key={selectedSignal.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="flex h-full flex-1 flex-col"><p className="mt-7 text-[10px] font-bold uppercase tracking-[0.18em] text-[#9DB8AD]">{selectedSignal.category} / {selectedSignal.move}</p><h3 className="mt-2 font-display text-[38px] leading-[0.96] tracking-[-0.03em]">{selectedSignal.signal}</h3><p className="mt-6 text-[15px] leading-7 text-[#E0E6DE]">{selectedSignal.detail}</p><div className="mt-auto border-l border-[#C99A48] bg-black/10 pl-4 pt-7"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#E0C27B]">Next confirmation</p><p className="mt-2 text-sm leading-6 text-[#F0E7D8]">{selectedSignal.watch}</p></div></motion.div></AnimatePresence>}
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="premarket" className="overflow-hidden border-y border-[#E1D7C5] bg-[#F9F5EB] px-5 py-16 sm:px-10 lg:px-14">
          <div className="mx-auto grid max-w-[1500px] gap-10 lg:grid-cols-[0.76fr_1.24fr]">
            <div><p className="section-kicker">08.18 / pre-market map</p><h2 className="section-title mt-3">Before the open, watch the proof — not the pitch.</h2><p className="mt-5 max-w-md text-sm leading-7 text-[#68756F]">A pre-market move is an invitation to investigate. The table shows the evidence that would strengthen or weaken each market narrative.</p>
            ...
            <div className="divide-y divide-[#DED3C2] border-t border-[#DED3C2]">
              {[
                ["Home Depot", "Same-store sales, pro demand, margin and full-year guide improve together.", "A cautious guide or promotion-led gross-margin pressure.", "Consumer / XLY"],
                ["Oil & Hormuz", "New talks and a narrower risk premium pull crude lower.", "Brent stays above $90 with visible shipping disruption.", "Macro / inflation"],
                ["Long-end yields", "Curve pressure eases and yields retrace.", "30Y makes fresh highs as term premium widens.", "Valuation / duration"],
                ["Fabrinet", "Cash-flow and guidance quality win acceptance in pre-market.", "The after-hours decline persists on margin or capex concern.", "AI hardware"],
              ].map(([event, positive, negative, exposure], index) => <article key={event} className="grid gap-3 py-5 sm:grid-cols-[140px_1fr_1fr] sm:gap-6"><div><p className="font-mono text-[10px] text-[#C99A48]">0{index + 1}</p><p className="mt-1 font-display text-xl">{event}</p><p className="mt-1 text-[11px] text-[#799087]">{exposure}</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#39785F]">Positive confirmation</p><p className="mt-2 text-sm leading-6 text-[#4F625B]">{positive}</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#AD5E52]">Negative confirmation</p><p className="mt-2 text-sm leading-6 text-[#4F625B]">{negative}</p></div></article>)}
            </div>
          </div>
        </section>
        ...
        <section className="bg-[#10342D] px-5 py-14 text-[#F7F0E3] sm:px-10 lg:px-14">
          <div className="mx-auto grid max-w-[1500px] gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div><p className="section-kicker text-[#E0C27B]">Research mode</p><h2 className="mt-3 max-w-3xl font-display text-[38px] leading-[1.02] sm:text-[50px]">{viewMode === "analysis" ? "This is a view with conditions, not a prediction." : "Learn how to read the signal before acting on the story."}</h2><p className="mt-5 max-w-2xl text-sm leading-7 text-[#D0DBD4]">{viewMode === "analysis" ? "Our current frame: long-end yields and oil are the market-wide risk switches; AI hardware retains selective demand visibility; retail guidance will determine whether consumer pressure is segmented or becoming broader." : "Use the same sequence every day: identify the macro driver, examine market breadth, separate sectors, test the data quality, then name the evidence that would change your mind."}</p></div>
            <div className="border-l border-[#C99A48] bg-white/[0.04] p-6"><div className="flex items-center gap-2 text-[#E0C27B]"><BookOpenText size={17} /><span className="font-mono text-[10px] uppercase tracking-[0.16em]">Briefing standard</span></div><p className="mt-4 font-display text-[27px] leading-tight">“Not broad risk-off — a style split led by long-end rates and geopolitics.”</p><p className="mt-4 text-sm leading-6 text-[#C5D0C9]">The concise wording is designed for a decision-maker conversation: state the frame, identify the evidence, then say what would invalidate it.</p></div>
          </div>
        </section>
