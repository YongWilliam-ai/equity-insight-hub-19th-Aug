  ...
  return (
    <main className="min-h-screen bg-[#F3EFE6] text-[#17372F] lg:flex">
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 border-b border-[#DED3C2] bg-[#F3EFE6]/95 px-5 py-3 backdrop-blur sm:px-8 lg:px-12"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4"><a href="#top" className="flex items-center gap-3"><ResearchMark className="h-9 w-9" /><span><strong className="block font-display text-[16px] leading-none">EQUITY / <em className="font-normal text-[#9A7230]">INSIGHT</em></strong><span className="font-mono text-[9px] tracking-[0.17em] text-[#9A7230]">BIT DAILY INTELLIGENCE</span></span></a><div className="hidden items-center gap-3 font-mono text-[10px] tracking-[0.12em] text-[#66766E] sm:flex"><span className="h-2 w-2 rounded-full bg-[#4E9377]" /> 简体中文主报告 <span className="text-[#B8AA95]">·</span> 2026.08.19</div><a href="/archive/2026-08-18" className="group inline-flex items-center gap-2 border border-[#D4C5AE] px-3 py-2 text-xs font-bold text-[#34544B] transition hover:border-[#A8782F] hover:text-[#9A7230]"><FileArchive size={15} /> 昨日归档 <ChevronRight size={14} className="transition group-hover:translate-x-0.5" /></a></div></header>
        ...
        <section id="brief" className="border-b border-[#DED3C2] bg-[#FBF7EE] px-5 py-9 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl"><Kicker>01 / 给上司的 60 秒版本 / 已核实</Kicker><div className="mt-5 grid gap-4 lg:grid-cols-3">{briefCards.map((card) => { const Icon = card.icon; return <article key={card.number} className="research-slip p-5"><div className="flex items-center justify-between"><span className="font-mono text-[10px] text-[#9A7230]">{card.number} / 证据摘录</span><Icon size={17} className="text-[#557A6C]" /></div><p className="mt-3 text-xs font-bold tracking-[0.12em] text-[#547067]">{card.label}</p><h2 className="mt-2 font-display text-[27px] leading-tight">{card.title}</h2><p className="mt-3 text-sm leading-6 text-[#68756F]">{card.body} <sup className="font-mono text-[#9A7230]">{card.citation}</sup></p></article>; })}</div></div></section>
