/**
 * The Market Ledger page — all user-visible copy derives from the active full-page language pack.
 */
import { useEffect, useMemo, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { ArrowRight, ArrowUpRight, BookOpenText, ChevronRight, Clock3, Filter, Menu, Search, ShieldCheck, Sparkles, TrendingDown, TrendingUp, X } from "lucide-react";
import { Bar, BarChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
import { breadthData, getMarketContent, type Direction, type Language, type MarketSignal, type ViewMode } from "@/lib/marketData";
...
export default function Home() {
  ...
  }, [language, copy.brandBottom, copy.brandTop, copy.researchEdition]);
  ...
  const categoryFor = (id: string) => ({ ALL: "", MACRO: copy.categories[1], AI: copy.categories[2], CONSUMER: copy.categories[3], CROSS: copy.categories[4] }[id] ?? "");
  const visibleSignals = useMemo(() => {
    const normalized = query.trim().toLowerCase();
    return signals.filter((signal) => {
      const categoryMatch = activeCategory === "ALL" || signal.category === categoryFor(activeCategory);
      const textMatch = !normalized || [signal.signal, signal.thesis, signal.lesson, signal.watch, signal.category].join(" ").toLowerCase().includes(normalized);
  ...
  return (
    <div className="min-h-screen bg-[#F3EFE6] text-[#16352F] lg:flex">
      <aside className="ledger-spine sticky top-0 z-40 hidden h-screen w-[236px] flex-col border-r border-[#E4DBC9] bg-[#10332D] px-6 py-7 text-[#F6F0E3] lg:flex">
        ...
        <div className="mt-14 border-l border-[#C99A48]/60 pl-4"><p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#C99A48]">{copy.edition}</p><p className="mt-3 whitespace-pre-line font-display text-[22px] leading-tight">{copy.sidebarThesis}</p></div>
        <nav className="mt-12 space-y-2" aria-label={copy.nav.join(" / ")}>
          {copy.nav.map((label, index) => (
            <button key={label} className="group flex w-full items-center gap-3 border-b border-white/10 py-3 text-left text-sm text-[#EAE3D5] transition hover:border-[#C99A48] hover:text-white" onClick={() => jumpTo(["overview", "signals", "premarket", "sources"][index])}>
              <span className="font-mono text-[10px] text-[#C99A48]">{String(index + 1).padStart(2, "0")}</span><span>{label}</span><ChevronRight size={14} className="ml-auto opacity-0 transition group-hover:opacity-100" />
            </button>
          ))}
        </nav>
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 border-b border-[#E4DBC9] bg-[#F3EFE6]/95 px-5 py-3 backdrop-blur lg:px-10"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3">
          <button className="flex items-center gap-2 lg:hidden" onClick={() => setMobileNavOpen(true)} aria-label={copy.openNav}><Menu size={21} /><img src={markImage} alt="" className="h-7 w-7 object-contain" /></button>
          <div className="hidden items-center gap-3 text-[11px] font-medium tracking-[0.14em] text-[#50615B] sm:flex"><span className="status-dot" /><span>{copy.locale}</span><span className="text-[#B6A997]">•</span><span>{copy.researchEdition}</span></div>
          <div className="ml-auto flex items-center gap-1 rounded-full border border-[#DED4C2] bg-[#FAF7F0] p-1" role="group" aria-label={copy.languageSwitcher}>
            {(["TW", "CN", "EN"] as Language[]).map((option, index) => <button key={option} onClick={() => switchLanguage(option)} className={`rounded-full px-2.5 py-1 text-[10px] font-bold transition ${language === option ? "bg-[#16352F] text-white" : "text-[#66746E] hover:text-[#16352F]"}`}>{languageNames[language][index]}</button>)}
          </div>
        </div></header>
        ...
        <main>
          ...
          <section className="border-y border-[#E2D8C6] bg-[#EAE4D8] px-5 py-14 sm:px-10 lg:px-14"><div className="mx-auto grid max-w-[1500px] gap-8 lg:grid-cols-[1.2fr_0.8fr]"><div className="relative overflow-hidden bg-[#173A33] p-7 text-[#F8F1E3] sm:p-9"><img src={macroImage} alt={copy.imageAltMacro} className="absolute inset-0 h-full w-full object-cover opacity-20" /><div className="relative"><p className="section-kicker text-[#E0C27B]">{copy.breadthKicker}</p><h2 className="mt-3 max-w-xl font-display text-[34px] leading-tight">{copy.breadthTitle}</h2><p className="mt-4 max-w-xl text-sm leading-6 text-[#D3D9D4]">{copy.breadthDescription}</p></div><div className="relative mt-8 h-[190px] border-l border-[#C99A48]/70 bg-[#0E2A25]/40 p-3"><ResponsiveContainer width="100%" height="100%"><BarChart data={breadthData} margin={{ top: 8, right: 10, left: -18, bottom: 0 }}><XAxis dataKey="market" tick={{ fill: "#D3D9D4", fontSize: 11, fontFamily: "IBM Plex Mono" }} axisLine={false} tickLine={false} /><YAxis domain={[0, 2]} ticks={[0, 1, 2]} tick={{ fill: "#9DB0A8", fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip cursor={{ fill: "rgba(255,255,255,0.05)" }} contentStyle={{ background: "#14342D", border: "1px solid rgba(224,194,123,0.45)", borderRadius: 0, color: "#F8F1E3", fontSize: 12 }} formatter={(value: number) => [`${value}:1`, copy.breadthAxisLabel]} /><Bar dataKey="ratio" fill="#C99A48" radius={[0, 0, 0, 0]} barSize={48} /></BarChart></ResponsiveContainer></div><p className="relative mt-4 font-mono text-[10px] tracking-[0.12em] text-[#AABAB3]">{copy.breadthSource}</p></div><div className="border-l border-[#C99A48] pl-6 sm:pl-8"><p className="section-kicker">{copy.whyKicker}</p><h3 className="mt-3 font-display text-[31px] leading-tight">{copy.whyTitle}</h3><div className="mt-7 space-y-5">{copy.whyItems.map((title, index) => <div key={title} className="border-b border-[#D4C9B8] pb-4"><p className="text-sm font-bold text-[#164038]">{title}</p><p className="mt-1 text-sm leading-6 text-[#69756F]">{copy.whyDescriptions[index]}</p></div>)}</div></div></div></section>

          <section id="signals" className="px-5 py-16 sm:px-10 lg:px-14"><div className="mx-auto max-w-[1500px]"><div className="grid gap-7 lg:grid-cols-[0.82fr_1.18fr] lg:items-end"><div><p className="section-kicker">{copy.signalsKicker}</p><h2 className="section-title mt-3">{copy.signalsTitle}</h2></div><p className="max-w-2xl text-sm leading-7 text-[#68756F]">{copy.signalsDescription}</p></div>
            <div className="mt-8 grid gap-7 xl:grid-cols-[1.12fr_0.88fr]"><div className="flex gap-0"><div className="hidden border-y border-l border-[#D7C6A7] bg-[#E7DED0] lg:flex lg:w-[42px] lg:flex-col lg:items-stretch">{categoryEntries.map(([id, label]) => <button key={id} onClick={() => setActiveCategory(id)} className={`signal-file-tab border-b border-[#D7C6A7] px-2 py-3 text-[9px] font-bold uppercase tracking-[0.13em] transition ${activeCategory === id ? "bg-[#153C34] text-[#F9F1E4]" : "text-[#627068] hover:bg-[#D5C29A] hover:text-[#153C34]"}`}>{label}</button>)}</div><div className="min-w-0 flex-1 lg:pl-5"><div className="flex flex-col gap-4 border-y border-[#DDD2C0] py-4 lg:flex-row lg:items-center lg:justify-between"><div className="relative flex-1"><Search className="absolute left-0 top-1/2 -translate-y-1/2 text-[#8B8A7C]" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={copy.searchPlaceholder} className="w-full border-0 border-b border-[#CFC3B0] bg-transparent py-3 pl-7 pr-3 text-sm text-[#15372F] outline-none placeholder:text-[#9B9B90] focus:border-[#C99A48]" /></div><div className="flex items-center gap-2 text-[10px] font-mono tracking-[0.14em] text-[#847968]"><Filter size={14} />{visibleSignals.length} {copy.signalsLabel}</div></div><div className="mt-4 flex flex-wrap gap-0 border-l border-t border-[#D7C6A7] lg:hidden">{categoryEntries.map(([id, label]) => <button key={id} onClick={() => setActiveCategory(id)} className={`border-b border-r border-[#D7C6A7] px-3 py-2 text-[10px] font-bold uppercase tracking-[0.1em] transition active:scale-[0.97] ${activeCategory === id ? "bg-[#153C34] text-white" : "bg-[#ECE5D9] text-[#6C766F] hover:bg-[#D5C29A] hover:text-[#153C34]"}`}>{label}</button>)}</div><div className="mt-5 divide-y divide-[#E1D7C5] border-t border-[#E1D7C5]">{visibleSignals.map((signal: MarketSignal, index: number) => <motion.button key={signal.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.035 }} onClick={() => setSelectedId(signal.id)} className={`group grid w-full grid-cols-[34px_1fr_auto] gap-3 py-5 text-left transition ${selectedSignal?.id === signal.id ? "bg-[#EBE5D9] px-3" : "hover:bg-[#F0EBE0] hover:px-3"}`}><span className="font-mono text-xs text-[#C99A48]">{String(signal.id).padStart(2, "0")}</span><span><span className="flex flex-wrap items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#658078]">{signal.category}</span><span className="text-[11px] text-[#887C69]">{signal.move}</span><span className="text-[9px] font-mono uppercase tracking-[0.12em] text-[#A47A34]">{isEducation ? copy.learningLens : copy.analystLens}</span></span><span className="mt-1 block font-display text-[22px] leading-tight text-[#16382F]">{isEducation ? `${copy.howToRead} ${signal.signal}` : signal.signal}</span><span className="mt-2 block max-w-xl text-sm leading-6 text-[#6A756F]">{isEducation ? signal.lesson : signal.thesis}</span></span><span className={`mt-1 flex h-7 w-7 items-center justify-center rounded-full border ${signal.direction === "up" ? "border-[#94B5A4] text-[#39775F]" : signal.direction === "down" ? "border-[#D4AAA2] text-[#AF584D]" : "border-[#D6C49D] text-[#9D7733]"}`}><DirectionIcon direction={signal.direction} /></span></motion.button>)}{visibleSignals.length === 0 && <div className="py-12 text-center text-sm text-[#7A827E]">{copy.emptySignals}</div>}</div></div></div>
              <div className="relative min-h-[490px] overflow-hidden bg-[#153A32] p-7 text-[#F7F0E3] sm:p-9"><img src={selectedSignal?.category === copy.categories[2] ? aiImage : macroImage} alt={copy.imageAltResearch} className="absolute inset-0 h-full w-full object-cover opacity-20 transition" /><div className="relative flex h-full flex-col"><div className="flex items-center justify-between border-b border-white/15 pb-4"><span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#E0C27B]">{isEducation ? copy.teachingNote : copy.researchDrawer}</span><Sparkles size={17} className="text-[#E0C27B]" /></div>{selectedSignal && <AnimatePresence mode="wait"><motion.div key={`${selectedSignal.id}-${viewMode}-${language}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="flex h-full flex-1 flex-col"><p className="mt-7 text-[10px] font-bold uppercase tracking-[0.18em] text-[#9DB8AD]">{selectedSignal.category} / {selectedSignal.move}</p><h3 className="mt-2 font-display text-[38px] leading-[0.96] tracking-[-0.03em]">{isEducation ? `${copy.whyItMatters} ${selectedSignal.signal}` : selectedSignal.signal}</h3><p className="mt-6 text-[15px] leading-7 text-[#E0E6DE]">{isEducation ? `${selectedSignal.lesson} ${selectedSignal.detail}` : selectedSignal.detail}</p><div className={`mt-auto border-l bg-black/10 pl-4 pt-7 ${isEducation ? "border-[#A4C2B3]" : "border-[#C99A48]"}`}><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#E0C27B]">{isEducation ? copy.nextRead : copy.invalidates}</p><p className="mt-2 text-sm leading-6 text-[#F0E7D8]">{isEducation ? selectedSignal.watch : selectedSignal.invalidation}</p></div></motion.div></AnimatePresence>}</div></div>
            </div></div></section>

          <section id="premarket" className="overflow-hidden border-y border-[#E1D7C5] bg-[#F9F5EB] px-5 py-16 sm:px-10 lg:px-14"><div className="mx-auto grid max-w-[1500px] gap-10 lg:grid-cols-[0.76fr_1.24fr]"><div><p className="section-kicker">{copy.premarketKicker}</p><h2 className="section-title mt-3">{isEducation ? copy.premarketEducationTitle : copy.premarketAnalysisTitle}</h2><p className="mt-5 max-w-md text-sm leading-7 text-[#68756F]">{isEducation ? copy.premarketEducationDescription : copy.premarketAnalysisDescription}</p><div className="mt-8 inline-flex border border-[#D8CCB7] p-1" role="group" aria-label={copy.readingMode}>{(["education", "analysis"] as ViewMode[]).map((mode) => <button key={mode} onClick={() => setViewMode(mode)} className={`px-4 py-2 text-xs font-bold transition ${viewMode === mode ? "bg-[#16382F] text-white" : "text-[#6A756F]"}`}>{mode === "education" ? copy.education : copy.analysis}</button>)}</div></div><div className="divide-y divide-[#DED3C2] border-t border-[#DED3C2]">{premarketRows.map((row, index) => <article key={row.event} className="grid gap-3 py-5 sm:grid-cols-[140px_1fr_1fr] sm:gap-6"><div><p className="font-mono text-[10px] text-[#C99A48]">{String(index + 1).padStart(2, "0")}</p><p className="mt-1 font-display text-xl">{row.event}</p><p className="mt-1 text-[11px] text-[#799087]">{row.exposure}</p></div>{isEducation ? <><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#39785F]">{copy.observe}</p><p className="mt-2 text-sm leading-6 text-[#4F625B]">{row.educationObserve}</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#AD5E52]">{copy.shortcut}</p><p className="mt-2 text-sm leading-6 text-[#4F625B]">{row.educationMistake}</p></div></> : <><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#39785F]">{copy.positive}</p><p className="mt-2 text-sm leading-6 text-[#4F625B]">{row.analysisPositive}</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#AD5E52]">{copy.negative}</p><p className="mt-2 text-sm leading-6 text-[#4F625B]">{row.analysisNegative}</p></div></>}</article>)}</div></div></section>
