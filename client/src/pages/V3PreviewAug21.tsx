/**
 * EQUITY / INSIGHT — V3 internal preview, 21 Aug 2026.
...
export default function V3PreviewAug21() {
  ...
  return <div className="v3-preview min-h-screen bg-[#F3EFE6] text-[#16352F] lg:flex">
    <aside className="ledger-spine sticky top-0 z-30 hidden h-screen w-[236px] flex-col border-r border-[#E4DBC9] bg-[#10332D] px-6 py-7 text-[#F6F0E3] lg:flex">
      ...
      <div className="mt-14 border-l border-[#C99A48]/60 pl-4"><p className="font-mono text-[10px] tracking-[0.2em] text-[#C99A48]">V3 INTERNAL PREVIEW / 2026.08.21</p><p className="mt-3 font-display text-[22px] leading-tight">先分市场，
再做传导验证。</p></div>
      ...
      <div className="mt-auto border border-white/15 bg-white/[0.04] p-4"><div className="flex items-center gap-2 text-[#C99A48]"><ShieldCheck size={15} /><span className="font-mono text-[10px] tracking-[0.16em]">INTERNAL / V3</span></div><p className="mt-2 text-xs leading-relaxed text-[#D5CEBF]">独立预览路由。原有首页与历史页面保持不变。</p></div>
    </aside>

    <div className="min-w-0 flex-1">
      <header className="sticky top-0 z-20 border-b border-[#E4DBC9] bg-[#F3EFE6]/95 px-5 py-3 backdrop-blur sm:px-10"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3"><a href="/" className="flex items-center gap-2 lg:hidden"><ResearchMark className="h-7 w-7" /><span className="font-display text-sm">EQUITY / INSIGHT</span></a><div className="hidden items-center gap-3 text-[11px] font-medium tracking-[0.14em] text-[#50615B] sm:flex"><span className="status-dot" /><span>简体中文内部预览</span><span className="text-[#B6A997]">•</span><span>V3 / 2026.08.21</span></div><a href="/" className="ml-auto inline-flex items-center gap-2 border border-[#D4C5AE] px-3 py-2 text-xs font-bold text-[#34544B] transition hover:border-[#A8782F] hover:text-[#9A7230]"><FileArchive size={14} /> 返回原版</a></div></header>
      ...
      <footer className="border-t border-[#E0D5C3] bg-[#F3EFE6] px-5 py-7 text-xs text-[#6D7871] sm:px-10 lg:px-14"><div className="mx-auto flex max-w-[1500px] flex-col justify-between gap-3 sm:flex-row"><p>EQUITY / INSIGHT · V3 INTERNAL PREVIEW · 2026.08.21</p><p>AI辅助内部市场研究；不构成个性化投资建议、证券推荐、买卖／沽空指令或任何回报保证。</p></div></footer>
    </div>
  </div>;
}
